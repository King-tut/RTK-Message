...
import {
  ...
  const Register = () => {
    ...
    const register = async (values, onSubmitProps) => {
      ...
      console.log(`This is from the register request ${formData.get('profilePic')}`)
      const savedUser = await savedUserResponse.json();
      onSubmitProps.resetForm();
  
      if (savedUser) {
        console.log(`Got the return from the server ${savedUser}`)
        setPageType("login");
      }
    };
      ...
      if (loggedIn) {
        dispatch(
          setLogin({
            ...
            currentUserID: loggedIn.user._id
          })
        );
        navigate("/messages");
      }
